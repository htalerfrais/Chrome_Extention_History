...
class RecallService:
    ...
    def ingest_clustered_session(self, user_id: int, session_identifier: str, clusters: List[Dict]) -> None:
        ...
        for cluster in clusters:
            ...
            existing = (topic.get("recall_state") or {}) if isinstance(topic, dict) else {}
            repetitions = int(existing.get("repetitions", 0)) + 1
            strength = min(1.0, float(existing.get("strength", 0.5)) + 0.08)
